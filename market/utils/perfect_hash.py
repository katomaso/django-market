"""Generate and decode an unique ID, which can use characters to shorten its length.
...
STRING_LENGTH = getattr(settings, 'MARKET_UID_LENGTH', None)


def find_suitable_period():
    """Automatically find a suitable period to use.

    Factors are best, because they will have 1 left over when dividing SIZE+1.
    ...
    # than say 4.
    starting = len(VALID_CHARS) > 14 and len(VALID_CHARS) // 2 or 13
    for p in itertools.chain(range(starting, 7, -1),
                             range(highest_acceptable_factor, starting + 1, -1),
                             [6, 5, 4, 3, 2]):
        if SIZE % p == 0:
            return p
...
def friendly_number(num):
    ...
    while STRING_LENGTH and len(hash) < STRING_LENGTH \
                or len(VALID_CHARS)**len(hash) <= SIZE:
        # PREpend string (to remove all obvious signs of order)
        hash = VALID_CHARS[int(num % len(VALID_CHARS))] + hash
        num = (num // len(VALID_CHARS))
    return hash


def encode(num):
    """Encode ``num`` to a more user friendly string of characters."""
    if num > SIZE:
        return None
    if num < 0:
        return None
